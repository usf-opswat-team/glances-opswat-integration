...
from glances.plugins.glances_processlist import split_cmdline
...
class _GlancesCurses(object):
    ...
    _hotkeys = {
        ...
        # 'L' > MetaDefender Cloud open scan report in browser
        'm': {'sort_key': 'memory_percent'},
        'M': {'switch': 'reset_minmax_tag'},
        'n': {'switch': 'disable_network'},
        'N': {'switch': 'disable_now'},
        # 'o': > MetaDefender Cloud scan selected process
        # 'O': > MetaDefender Cloud scan all processes
        'p': {'sort_key': 'name'},
    ...
    def __init__(self, config=None, args=None):
        ...
        # Init MetaDefender Cloud scan process tag
        self.mdc_scan_process = False

        # Init MetaDefender Cloud scan process tag
        self.mdc_scan_all_processes = False

        # Init MetaDefender Cloud view report tag
        self.mdc_report_process = False
    ...
    def __catch_key(self, return_to_browser=False):
        ...
        if self.pressedkey == ord('\n'):
            ...
            # 'L' > Scan selected process (after confirmation)
            self.mdc_report_process = not self.mdc_report_process
        elif self.pressedkey == ord('o'):
            # 'o' > Scan selected process (after confirmation)
            self.mdc_scan_process = not self.mdc_scan_process
        elif self.pressedkey == ord('O'):
            # 'o' > Scan selected process (after confirmation)
            self.mdc_scan_all_processes = not self.mdc_scan_all_processes
    ...
    def display(self, stats, cs_status=None):
        ...
        # Display scan process confirmation popup
        # Only in standalone mode (cs status is None)
        if self.mdc_scan_process and cs_status is None:
            self.mdc_scan(stats.get_plugin('processlist').get_raw()[self.args.cursor_position])
        elif self.mdc_scan_process and cs_status is not None:
            self.display_popup('Scan process only available for local processes')
        self.mdc_scan_process = False

        # Display scan process confirmation popup
        # Only in standalone mode (cs status is None)
        if self.mdc_scan_all_processes and cs_status is None:
            self.mdc_scan_all(stats.get_plugin('processlist').get_raw())
        elif self.mdc_scan_all_processes and cs_status is not None:
            self.display_popup('Scan process only available for local processes')
        self.mdc_scan_all_processes = False

        # Display scan process confirmation popup
        # Only in standalone mode (cs status is None)
        if self.mdc_report_process and cs_status is None:
            self.mdc_report(stats.get_plugin('processlist').get_raw()[self.args.cursor_position])
        elif self.mdc_report_process and cs_status is not None:
            self.display_popup('MDC view report only available for local processes')
        self.mdc_report_process = False
    ...
    def mdc_scan(self, process):
        """Scan a process with OPSWAT's MetaDefender Cloud API

        :param process
        :return: None
        """
        logger.debug("Selected process to scan: {}".format(process))
        bare_process_name = process['name']
        cmdline = process.get('cmdline', '?')
        path, cmd, arguments = split_cmdline(bare_process_name, cmdline)
        filepath_to_scan = path + "/" +  cmd
        ...
        if confirm.lower().startswith('y'):
            print(" scanning...")
            ret_mdc_scan = glances_processes.mdc_scan(filepath_to_scan, process)
            if ret_mdc_scan == 0:
                            self.display_popup('The process to scan does not have an associated filepath. \nPress "Enter" to close this message', popup_type='yesno')                
            if ret_mdc_scan == None:
                            self.display_popup('Invalid OPSWAT API key set.\nPlease provide an API key in the conf/opswat.json file.\nYou can get an API key here: https://id.opswat.com\nPress "Enter" to close this message', popup_type='yesno')
            elif ret_mdc_scan == 429:
                self.display_popup('Please try again later or obtain an API key with higher limits. \nYou can go to OPSWAT website to register more subcriber: https://metadefender.opswat.com/licensing \nPress "Enter" to close this message', popup_type='yesno')

    def mdc_scan_all(self, processes):
        """Scan a process with OPSWAT's MetaDefender Cloud API"""
        confirm = self.display_popup(
            'Scan all processes?\n\nConfirm ([y]es/[n]o):\n',
            popup_type='yesno',
        ...
        if confirm.lower().startswith('y'):
            print(" scanning all...")
            t = Thread(target=self.mdc_scan_all_thread, args=[processes])
            t.start()
            return

    def mdc_scan_all_thread(self, processes):
        for proc in processes:
                try:
                    bare_process_name = proc['name']
                    cmdline = proc.get('cmdline', '?')
                    path, cmd, arguments = split_cmdline(bare_process_name, cmdline)
                    filepath_to_scan = path + "/" + cmd
                    ret_mdc_scan = glances_processes.mdc_scan(filepath_to_scan, proc, True)
                    if ret_mdc_scan == None:
                        self.display_popup('Invalid OPSWAT API key set.\nPlease provide an API key in the conf/opswat.json file.\nYou can get an API key here: https://id.opswat.com\nPress "Enter" to close this message', popup_type='yesno')
                        return
                    elif ret_mdc_scan == 429:
                    ...
                    logger.debug("Failed to scan process: {}".format(proc))
    ...
    def mdc_report(self, process):
        ...
        :return: None
        """
        logger.debug("Selected process to view MDC report: {}".format(process))

        if 'mdc' not in process or process['mdc']['hash'] == None:
            self.display_popup('Unable to open MDC report for unscanned process.\nPlease scan with "o" and try again. \nPress "Enter" to close this message', popup_type='yesno')
            return
        ...
        if confirm.lower().startswith('y'):
            glances_processes.mdc_report(process)
            # Success
            # if ret_mdc_scan == 0:
            #     logger.info('Scan signal has been sent to process {} {} (return code: {})'.format(pids, filepath_to_scan, ret_mdc_scan))
            # else:
            #     logger.error('Can not scan process {} {}'.format(pids, filepath_to_scan))
