...
class Plugin(GlancesPlugin):
    ...
    def _get_process_curses_mdc(self, p, selected, args):
        """Return process MDC curses"""
        if 'mdc' in p:
            # docker internal users are displayed as ints only, therefore str()
            # Correct issue #886 on Windows OS
            msg = self.layout_stat['mdc'].format(p['mdc']['label'])
            mdc_decoration = "DEFAULT"
            if 'scan_all_result_i' in p['mdc']:
                if p['mdc']['scan_all_result_i'] == 0:
                    mdc_decoration = "OK"
                elif p['mdc']['scan_all_result_i'] == 1:
                    mdc_decoration = "CRITICAL"
                else:
                    mdc_decoration = "WARNING"
            ret = self.curse_add_line(msg, decoration=mdc_decoration)
        else:
            msg = self.layout_header['mdc'].format('?')
            ret = self.curse_add_line(msg)
        return ret
    ...
    def get_process_curses_data(self, p, selected, args):
        ...
        # MDC (MetaDefender Cloud)
        ret.append(self._get_process_curses_mdc(p, selected, args))
    ...
    def __msg_curse_header(self, ret, process_sort_key, args=None):
        ...
        msg = self.layout_header['mdc'].format('MDC')
        ret.append(self.curse_add_line(msg))
        if not self.args.programs:
